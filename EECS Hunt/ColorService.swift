...
class ColorService : NSObject {
    ...
    func send(hint : String) {
        NSLog("%@", "sendColor: \(hint) to \(session.connectedPeers.count) peers")

        if session.connectedPeers.count > 0 {
            do {
                try self.session.send(hint.data(using: .utf8)!, toPeers: session.connectedPeers, with: .reliable)
            }
            catch let error {
...
extension ColorService : MCSessionDelegate {
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        ...
        self.delegate?.hintChanged(manager: self, hint: str)
    }
